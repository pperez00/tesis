...
try {
    if (strpos($dato, '_') !== false && intval($id_usuario) > 0) {
        ...
        $archivo_buscar = $archivo->get_archivo_grupo($id_archivo);
        if(count($archivo_buscar) > 0 && count($grupo->get_grupo(" and id_chat_grupo='" . $id_grupo . "'")) > 0){
           ...
           if(file_exists('../usuarios/' . $ruta) ) {
            $enviar['ruta'] = $pdoconnect->get_ruta() . '/usuarios/' . $ruta;
            $enviar['archivo'] = $archivo_buscar[0]['archivo'];
            $msj = 'Archivo descargado';
           } else {
            $error = true;
